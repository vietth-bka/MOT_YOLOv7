...
def build_resnet_backbone(cfg):
    ...
    model = ResNet(last_stride, bn_norm, with_ibn, with_se, with_nl, block,
                   num_blocks_per_stage, nl_layers_per_stage, input_channel)
    if pretrain:
        # Load pretrain path if specifically
        cached_file = pretrain_path
        if pretrain_path:
            try:
            ...
            state_dict, cached_file = init_pretrained_weights(key)
        ...
        if len(matched_layers) == 0:
            ...
            print(
                'Successfully loaded resnet pretrained weights from "{}"'.
                format(cached_file)
            )
